...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ word: typeof result.word === "string" ? result.word.trim().slice(0, 120) : word, phonetic: typeof result.phonetic === "string" ? result.phonetic.trim().slice(0, 120) : "", wordType, meaning, cefrLevel: getCefrLevel(word) || "未收录" });
  } catch (error) {
