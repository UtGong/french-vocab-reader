...
export async function POST(request: Request) {
  try {
    ...
    const result = await askLanguageModel(`Write one natural modern French sentence suitable for CEFR ${targetLevel}, using all these learned vocabulary items exactly or in a grammatically inflected form: ${JSON.stringify(words)}. Maximize reuse of these learned words. Content words should come from this list whenever grammatically possible; use outside vocabulary only when the sentence cannot be grammatical or natural without it, and then use the absolute minimum. Function words such as articles, pronouns, prepositions, conjunctions, and auxiliaries are allowed. Return exactly JSON: {"sentence":"French sentence","translation":"accurate Simplified Chinese translation","usedWords":["learned items actually used"],"note":"concise Simplified Chinese note that also identifies any necessary outside content word"}. Do not add other text.`, 700);
