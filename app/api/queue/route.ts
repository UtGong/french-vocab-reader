...
export async function GET() {
  try {
    ...
    const rows = await sql`SELECT id, word, phonetic, word_type_zh, meaning_zh, details_zh, source_word, created_at FROM study_queue WHERE user_id = ${user.id} ORDER BY created_at DESC`;
...
export async function POST(request: Request) {
  try {
    ...
    for (const item of items) {
      ...
      const details = clean(item.details, 1000), sourceWord = clean(item.sourceWord, 120), phonetic = clean(item.phonetic, 120);
      ...
      if (existing.length) {
        await sql`UPDATE study_queue SET word = ${word}, phonetic = ${phonetic}, word_type_zh = ${wordType}, meaning_zh = ${meaning}, details_zh = ${details}, source_word = ${sourceWord}, created_at = NOW() WHERE id = ${existing[0].id} AND user_id = ${user.id}`;
      } else {
        await sql`INSERT INTO study_queue (user_id, word, phonetic, word_type_zh, meaning_zh, details_zh, source_word) VALUES (${user.id}, ${word}, ${phonetic}, ${wordType}, ${meaning}, ${details}, ${sourceWord})`;
      }
      saved += 1;
    }
