...
function VocabularyApp({ email, logout }: { email: string; logout: () => Promise<void> }) {
  ...
  return <main>
    ...
    <section className="card">
      <nav className="primary-switch five-tabs"><button className={mode === "text" ? "active" : ""} onClick={() => setMode("text")}>法语文本</button><button className={mode === "explore" ? "active" : ""} onClick={() => setMode("explore")}>词汇联想</button><button className={mode === "dictionary" ? "active" : ""} onClick={() => setMode("dictionary")}>法语词典</button><button className={mode === "sentence" ? "active" : ""} onClick={() => setMode("sentence")}>趣味短篇</button><button className={mode === "knowledge" ? "active" : ""} onClick={() => setMode("knowledge")}>词汇图谱</button></nav>
      ...
      </section> : mode === "explore" ? <VocabExplorer onQueued={loadQueue} targetLevel={targetLevel} /> : mode === "dictionary" ? <Dictionary onUpdated={() => { loadQueue(); loadLearned(); }} /> : mode === "sentence" ? <SentencePractice targetLevel={targetLevel} onUpdated={() => { loadQueue(); loadLearned(); }} /> : <KnowledgeGraph learned={learned} onReview={(items) => beginStudy(items, "review")} />}
      {mode !== "knowledge" && <>
      <StudyTimer />
